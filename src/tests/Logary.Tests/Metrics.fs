...
let snapshot =
  ...
  testList "calculating snapshot values" [
    ...
    testCase "has values" <| fun _ ->
      Assert.equal (Snapshot.values sample) [| 1L; 2L; 3L; 4L; 5L |] "should have values ordered"
      ...
      let testEWMA explaination instance (expectations : _ list) =
        ...
        testCase explaination <| fun _ ->
          ...
          |> List.iteri (fun index (expected, actual) ->
            Assert.floatEqual actual expected (Some 0.00000001) (sprintf "Index %d, should calculate correct EWMA" index))
