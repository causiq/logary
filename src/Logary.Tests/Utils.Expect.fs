...
let sequenceContainsOrder message (expectedSub: #seq<'t>) (actual: #seq<'t>) =
  ...
  let rec iter i =
    ...
    if not (ae.MoveNext()) then
      failtestf
        "%s. Remainder of expected enumerable:%s%s%sWent through actual enumerable (%i items):%s%s"
        message nl (printSeq el) nl i nl (printSeq al)
    else
      al.Add ae.Current
