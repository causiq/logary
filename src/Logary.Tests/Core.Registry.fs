...
let debug (message: string) = testLogger.logBP(Model.Event(message, level=Debug)) ^-> ignore
...
let tests =
  testList "registry" [
    testCaseJob "from Config and Multi shutdown" <| job {
      ...
      let! logm =
        Config.create(Resource.create("svc", "localhost"))
        |> Config.ilogger (ILogger.Console Verbose)
        |> Config.build
      do! debug "Started LogManager."

      let ri = logm.runtimeInfo
      ri.resource.service |> Expect.equal "should have service" "svc"
    ...
    testCaseJob "flush/shutdown timeout and logging after shutdown with no blocking" (job {
      ...
      let! logm =
        Config.create(Resource.create("svc", "host"))
      ...
      let! res = lg.logBP (Model.Event "Can be successfully logged")
      res
      ...
      let! res = lg.logAck (Model.Event "Cannot be logged because the registry from which the logger comes, is closed.")
      res
        |> Expect.isFalse "Should fail to log, asserting on Alt<bool>-API"

      match! lg.logWithAck (true, Model.Event "Second try") with
      | Ok _ ->
