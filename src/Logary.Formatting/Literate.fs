...
module Literate =
  ...
  let tokeniseExceptions (pvd: IFormatProvider) (nl: string) (m: LogaryMessageBase) =
    let exceptions =
  ...
  let tokeniseTemplateByGauges (pvd: IFormatProvider) (gauges: IDictionary<string, Gauge>) =
    let gauges =
      Array.ofSeq gauges
      |> Array.sortBy fst
    if Array.isEmpty gauges then
      ...
      seq {
        ...
        for i = 0 to lastIndex do
          ...
          match units with
          | U.Scaled (Seconds, scale) when scale = float Constants.NanosPerSecond ->
  ...
  let tokeniseTemplateByFields (pvd: IFormatProvider) (template: Template) (resolver: DestructureResolver) (fields: seq<string * obj>) =
    ...
    let tryGetPropertyValue (pt: Logary.Internals.FsMtParserFull.Property) =
      match Map.tryFind pt.name fieldsMap with
      | Some value ->
  ...
  let tokeniseTemplateWithGauges (pvd: IFormatProvider) destr (message: LogaryMessageBase) =
    let tplByGauges = tokeniseTemplateByGauges pvd message.gauges
